...
class VAE(npfl138.TrainableModule):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        encoder_layers_list = []
        encoder_layers_list.append(torch.nn.Flatten())
        last_dim = MNIST.C * MNIST.H * MNIST.W
        for hidden_dim in args.encoder_layers:
            encoder_layers_list.append(torch.nn.Linear(last_dim, hidden_dim))
            encoder_layers_list.append(torch.nn.ReLU())
            last_dim = hidden_dim
        # Output layer for mean and log_sd (log variance is also common)
        encoder_layers_list.append(torch.nn.Linear(last_dim, 2 * self._z_dim))
        self.encoder = torch.nn.Sequential(*encoder_layers_list)
        ...
        decoder_layers_list = []
        last_dim = self._z_dim
        for hidden_dim in args.decoder_layers:
            decoder_layers_list.append(torch.nn.Linear(last_dim, hidden_dim))
            decoder_layers_list.append(torch.nn.ReLU())
            last_dim = hidden_dim
        # Output layer to reconstruct the image
        decoder_layers_list.append(torch.nn.Linear(last_dim, MNIST.C * MNIST.H * MNIST.W))
        decoder_layers_list.append(torch.nn.Sigmoid())  # Use Sigmoid for output in [0, 1] range
        # Reshape the output to image dimensions
        decoder_layers_list.append(torch.nn.Unflatten(1, (MNIST.C, MNIST.H, MNIST.W)))
        self.decoder = torch.nn.Sequential(*decoder_layers_list)
    ...
    def train_step(self, xs: tuple[torch.Tensor], y: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        # The second half is log(z_sd^2) or log(z_sd) depending on convention.
        # Assuming log(z_sd) based on comment "logarithm of z_sd"
        z_log_sd = encoder_output[:, self._z_dim:]
        z_sd = torch.exp(z_log_sd)  # Calculate standard deviation
        ...
        # by the distribution.
        q_z_x = torch.distributions.Normal(z_mean, z_sd)
        z = q_z_x.rsample()  # Sample z ~ q(z|x) using reparametrization trick
        ...
        reconstruction_loss = torch.nn.functional.binary_cross_entropy(
            decoded_images.view(batch_size, -1),
            images.view(batch_size, -1),
            reduction='sum' # Sum over all pixels for each image
        ...
        p_z = self._z_prior()
        latent_loss = torch.mean(torch.sum(torch.distributions.kl.kl_divergence(q_z_x, p_z), dim=1))
        ...
        reconstruction_loss_per_pixel_mean = torch.nn.functional.binary_cross_entropy(
            decoded_images.view(batch_size, -1), images.view(batch_size, -1), reduction='mean'
        )
