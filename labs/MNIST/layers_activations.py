...
def main(args: argparse.Namespace) -> dict[str, float]:
    ...
    for i in range(args.hidden_layers):
        if i>0:
            ...
            layers.append(torch.nn.Linear(MNIST.W * MNIST.H, args.hidden_layer_size))
        if args.activation != "none":
            layers.append(activation_map[args.activation])

    # Add the output layer.
    if args.hidden_layers > 0:
        layers.append(torch.nn.Linear(args.hidden_layer_size, MNIST.LABELS))
    else:
        layers.append(torch.nn.Linear(MNIST.W * MNIST.H, MNIST.LABELS))
