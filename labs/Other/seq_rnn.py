...
class Model(npfl138.TrainableModule):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        rnn_layer = getattr(torch.nn, args.rnn)(
            input_size=args.sequence_dim, hidden_size=args.rnn_dim,
        ...
        if args.hidden_layer:
            self.hidden_layer = torch.nn.Linear(args.rnn_dim, args.hidden_layer)
        ...
        self.output_layer = torch.nn.Linear(
            args.hidden_layer if args.hidden_layer else args.rnn_dim, 1
        )
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        if self.hidden_layer:
            rnn_out = self.hidden_activation(self.hidden_layer(rnn_out))

        output = self.sigmoid(self.output_layer(rnn_out))
        return output
