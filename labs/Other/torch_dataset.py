...
parser.add_argument("--augment", default=True, action="store_true", help="Whether to augment the data.")
...
class TransformedDataset(npfl138.TransformedDataset):
    ...
    def transform(self, example: dict[str, torch.Tensor]):
        ...
        example["image"] = example["image"].float() / 255
        if self._augmentation_fn:
            example["image"] = self._augmentation_fn(example["image"])
