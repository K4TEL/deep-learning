...
class Model(torch.nn.Module):
    ...
    def train_epoch(self, dataset: MNIST.Dataset) -> None:
        ...
        for batch in dataset.batches(self._args.batch_size, shuffle=True):
            ...
            #     and then compute the crossentropy from the definition
            ...
            one_hot_labels = torch.nn.functional.one_hot(labels.type(torch.int64), num_classes=10)
            loss = -torch.sum(one_hot_labels * torch.log(probabilities), dim=1).mean()
            ...
            with torch.no_grad():
                for parameter, gradient in zip([self._W1, self._b1, self._W2, self._b2], gradients):
